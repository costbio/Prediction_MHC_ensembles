...
def rmsd_summary(pdb_file, traj_path, atom_selection):
    traj = md.load_xtc(traj_path, top=pdb_file)
    ref = md.load_pdb(pdb_file)
...
def rmsd_gen(pdb_file, traj_path, atom_selection ):
    ...
    traj = md.load_xtc(traj_path, top=ref)
    traj.superpose(ref)
    vals = md.rmsd(traj, ref)
...
# ============================================================
def run_pipeline(
    xtc_file, pdb_file, atom_selection, out_dir,
    ...
    traj, ref = load_and_align(xtc_file, pdb_file, atom_selection)
    ...
    gen_xtc_path = os.path.join(out_dir, "generated_test.xtc")
    save_xtc(ref, decoded_xyz, gen_xtc_path)

    real_rmsd = rmsd_summary(pdb_file, xtc_file, atom_selection)
    gen_rmsd = rmsd_gen(pdb_file, gen_xtc_path, atom_selection)
...
if __name__ == "__main__":
    ...
    run_pipeline(
        args.xtc, args.pdb, args.atom_sel, args.out_dir,
